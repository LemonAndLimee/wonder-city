...
public class GameManagement : MonoBehaviour
{
    ...
    public void CarSpawn()
    {
        ...
        CarController carScript = currentCar.GetComponent<CarController>();
        carScript.rows = rows;
        int number = Random.Range(1, 3);
        if (number == 1)
        {
            currentCar.transform.position = new Vector3(currentCar.transform.position.x, currentCar.transform.position.y, -rows);
            carScript.upwards = true;
        }
        else
        {
            currentCar.transform.position = new Vector3(currentCar.transform.position.x + (float)0.213, currentCar.transform.position.y, (float)-0.8);
            carScript.upwards = false;
            Quaternion carRot = new Quaternion();
            carRot.y = 180;
            currentCar.transform.localRotation = carRot;
        }
    }
